/*
  This WAS an Arduino library written for the NAU7802 24-bit wheatstone
...
bool NAU7802::begin(int &i2cPort, uint8_t i2cAddress, bool initialize)
{
  //Get user's options
  _i2cPort = i2cPort;
  ...
  char filename[20];
  snprintf(filename, 19, "/dev/i2c-%d", _i2cPort);
  _file = open(filename, O_RDWR);
  if (_file < 0) {  //open returns < 0 if unable to open the file
    return false;
  }
  //I2C device "opened", but ioctl just specifies the device on the bus to communicate with
  if (ioctl(_file, I2C_SLAVE, _deviceAddress) < 0) {  //_deviceAddress showing up via i2cdetect is irrelevant
    return false;
  }

  __s32 response = i2c_smbus_read_byte(_file);  //returns < 0 if failure
  if (response < 0) {
    return false;
  }
  ...
  if (initialize)
  {
    ...
    if(!powerUp()){  //Power on analog and digital sections of the scale
      return false;
    }
    if(!setLDO(NAU7802_LDO_3V3)){  //Set LDO to 3.3V
      return false;
    }
    if(!setGain(NAU7802_GAIN_128)){  //Set gain to 128
      return false;
    }
    if(!setSampleRate(NAU7802_SPS_80)){  //Set samples per second to 10
      return false;
    }
    if(!setRegister(NAU7802_ADC, 0x30)){  //Turn off CLK_CHP. From 9.1 power on sequencing.
      return false;
    }
    if(!setBit(NAU7802_PGA_PWR_PGA_CAP_EN, NAU7802_PGA_PWR)){  //Enable 330pF decoupling cap on chan 2. From 9.14 application circuit note.
      return false;
    }
    if(!calibrateAFE()){  //Re-cal analog front end when we change gain, sample rate, or channel
      return false;
    }
  }
...
bool NAU7802::waitForCalibrateAFE(uint32_t timeout_ms)
{
  ...
  while ((cal_ready = calAFEStatus()) == NAU7802_CAL_IN_PROGRESS)
  {
    ...
    auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - begin);
...
int32_t NAU7802::getReading()
{
  ...
  uint8_t dataPtr[3];
  __s32 data;
  data = i2c_smbus_read_i2c_block_data(_file, (uint8_t) NAU7802_ADCO_B2, 3, dataPtr);  //returns < 0 if failure
  if (data < 0) {
    return -1;
  }

  valueRaw = ((uint32_t)(dataPtr[0] << 16)) | ((uint32_t)(dataPtr[1] << 8)) | ((uint32_t)dataPtr[2]);
...
{
  __s32 data;
  data = i2c_smbus_read_word_data(_file, registerAddress);  //returns < 0 if failure
  if (data < 0) {
    return -1;
  }
...
{
  __s32 data;
  data = i2c_smbus_write_word_data(_file, registerAddress, value);  //returns < 0 if failure
  if (data < 0) {
    return false;
  }
  return true;
