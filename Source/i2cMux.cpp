...
extern "C" {
  #include <i2c/smbus.h>
  #include <linux/i2c-dev.h>    //sudo apt install libi2c-dev (add "-l i2c" to compilation)
}
...
bool QWIICMUX::begin(int i2cPort, uint8_t i2cAddress)
{
  //Get user's options
  _i2cPort = i2cPort;
...
bool QWIICMUX::isConnected()
{
  ...
  if (ioctl(_file, I2C_SLAVE, _deviceAddress) < 0) {
    return false;
  }
...
//If port number is out of range, disable all ports
bool QWIICMUX::setPort(uint8_t portNumber)
{
  ...
  __s32 res = i2c_smbus_write_byte(_file, portValue);
  if (res < 0) {
    return false; 
  }
  return true;
...
uint8_t QWIICMUX::getPort()
{
  ...
  uint8_t portBits;
  // portBits = static_cast<uint8_t>(i2c_smbus_read_byte(_file));  //returns < 0 if failure
  portBits = i2c_smbus_read_byte(_file);  //returns < 0 if failure
...
{
  __s32 res = i2c_smbus_write_byte(_file, portBits);
  if (res < 0) {
    return false; 
  }
  return true;
...
uint8_t QWIICMUX::getPortState()
{
  __s32 portBits;
  portBits = i2c_smbus_read_byte(_file);  //returns < 0 if failure
  return static_cast<uint8_t>(portBits);
}
